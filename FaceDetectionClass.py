...
class FaceDetectionApp:
    ...
    def setup_image_tab(self):
        ...
        self.detect_button = ttk.Button(
            self.image_tab, text="Detect Faces", command=self.detect_faces, state=tk.DISABLED
        )
        self.detect_button.pack(pady=10)

        self.save_button = ttk.Button(
            self.image_tab, text="Save Image", command=self.save_image, state=tk.DISABLED
        )
    ...
    def setup_video_tab(self):
        ...
        self.passw_label = tk.Label(self.video_tab, text='Video Source ID', font=('calibre', 10, 'bold'))
        self.passw_label.pack(pady=10, padx=10)
        self.video_source_entry.pack(pady=10)


        self.start_video_button = ttk.Button(
            self.video_tab, text="Start Video Processing", command=self.start_video_processing
        )
        self.start_video_button.pack(pady=10)

        self.stop_video_button = ttk.Button(
            self.video_tab, text="Stop Video Processing", state=tk.DISABLED,
    ...
    # Saving the Image locally after showing the detection bounding box
    def save_image(self):
        if self.image is None:
    ...
    def start_video_processing(self):
        ...
        self.video_loop()   # NO THREADS → smooth video
    ...
    def stop_video_processing(self):
        ...
        print("Sending all the detected faces snapshots. Don't close the window")
        send_all_at_once()
    ...
    # Keeps the Video Looping
    def video_loop(self):
        """Smooth flicker-free video processing loop using .after()"""
        ...
        for (x, y, w, h) in faces:
            ...
            if duration > self.presence_limit.get():
                color = (0, 0, 255)
                cv2.putText(frame, "ALERT: Too long!", (x, y - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)

                # Save image if not already saved
                datetime_string = datetime.now().strftime("%Y-%m-%d %H-%M-%S")
                img_name = f"detected_images/Detected_ID-{matched_id}@{datetime_string}.png"
                pattern = re.compile(rf"^Detected_ID-{matched_id}.*\.png$")
                directory = Path("detected_images")

                if not directory.exists():
                    os.makedirs(directory)

                found = any(pattern.match(f.name) for f in directory.glob("*.png"))
                if not found:
                    cv2.imwrite(img_name, frame)

            cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
            cv2.putText(frame, f"ID {matched_id} | {duration:.1f}s",
    ...
    # Render Image frames continuously as a video
